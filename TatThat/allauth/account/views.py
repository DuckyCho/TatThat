...
from user.models import SubscribeUser
...
class PasswordChangeView(AjaxCapableProcessFormViewMixin, FormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        if not request.user.has_usable_password():
            return HttpResponseRedirect(reverse('account_social_user_change_password'))
...
class AccountIndexView(TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        if request.POST:
            ...
            return super(AccountIndexView, self).dispatch(request, *args, **kwargs)


    def get_context_data(self, **kwargs):
        context = super(AccountIndexView,self).get_context_data(**kwargs)
        context['isAccountIndexView'] = True
        scrap_pics = Scrap.objects.filter(user_id=self.request.user.id)
        subscribe_check = SubscribeUser.objects.filter(user_id=self.request.user.id)
        context['scrap_count'] = len(scrap_pics)
        if subscribe_check:
            context['isSubscribe'] = True
        return context
